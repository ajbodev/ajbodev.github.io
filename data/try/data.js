data({
  "data": [
    {
      "name": "lklk",
      "input_html": "{\"index\":{\"name\":\"index\",\"ext\":\"html\",\"mode\":\"text/html\",\"src\":\"<!DOCTYPE html>\\n<html>\\n  <head>\\n    <style data-include='index'></style>\\n  </head>\\n  <body>Please please NoMOOORERERE\\n    <div id='app'></div>\\n    <script data-include='index'></script>\\n  </body>\\n</html>\"}}",
      "input_css": "{\"index\":{\"name\":\"index\",\"ext\":\"css\",\"mode\":\"text/css\",\"src\":\"\\n#app {\\n  background: red;\\n  height: 50px;\\n  width: 50px;\\n}\\n\"}}",
      "input_js": "{\"index\":{\"name\":\"index\",\"ext\":\"js\",\"mode\":\"text/javascript\",\"src\":\"\\nalert(1);\\n\"}}",
      "date": "23/07/2016",
      "id": 1
    },
    {
      "name": "pop",
      "input_html": "{\"index\":{\"name\":\"index\",\"ext\":\"jade\",\"mode\":\"text/x-jade\",\"src\":\"doctype html\\nhtml\\n  body adas \\n    div#app\\n    script(data-include='index')\"},\"iioioio\":{\"name\":\"iioioio\",\"ext\":\"html\",\"mode\":\"text/html\",\"src\":\"\\n\\n\"}}",
      "input_css": "{\"index\":{\"name\":\"index\",\"ext\":\"css\",\"mode\":\"text/css\",\"src\":\"\\n#app {\\n  background: red;\\n  height: 50px;\\n  width: 50px;\\n}\\n\"}}",
      "input_js": "{\"index\":{\"name\":\"index\",\"ext\":\"js\",\"mode\":\"text/javascript\",\"src\":\"\\nalert(1);\\n\"}}",
      "date": "23/07/2016",
      "id": 2
    },
    {
      "name": "popop",
      "input_html": "{\"index\":{\"name\":\"index\",\"ext\":\"jade\",\"mode\":\"text/x-jade\",\"src\":\"doctype html\\nhtml\\n  body adas\\n    div#app\\n    script(data-include='index')\"},\"iioioio\":{\"name\":\"iioioio\",\"ext\":\"html\",\"mode\":\"text/html\",\"src\":\"\\n\\n\"}}",
      "input_css": "{\"index\":{\"name\":\"index\",\"ext\":\"css\",\"mode\":\"text/css\",\"src\":\"\\n#app {\\n  background: red;\\n  height: 50px;\\n  width: 50px;\\n}\\n\"}}",
      "input_js": "{\"index\":{\"name\":\"index\",\"ext\":\"js\",\"mode\":\"text/javascript\",\"src\":\"\\nalert(1);\\n\"}}",
      "date": "26/07/2016",
      "id": 3
    }
  ]
})